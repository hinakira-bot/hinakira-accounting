...
【初回セットアップ（重要）】
1. 「Googleでログイン」ボタンでGoogleアカウントでログインする
2. 「Gemini APIキー」の設定が必須。右上の歯車アイコン（設定）をクリックして設定画面を開く
3. Google AI Studio（https://aistudio.google.com/app/apikey）にアクセスしてAPIキーを取得
4. 設定画面にAPIキーを貼り付けて「設定を保存」をクリック
※ APIキーがないとAI自動判定・証憑読み取り・このチャット機能は全て使えません
※ ログイン時にGoogle Driveに証憑保管用フォルダ（Accounting_Evidence）が自動作成されます

【グローバル年度セレクタ】
・ TOPメニュー画面の左上に年度選択（プルダウン）があります
・ 選択した年度が全画面に適用されます（仕訳入力の日付範囲、アウトプットの期間、証憑読み取りの年度チェックに連動）
・ 年度はlocalStorageに保存され、次回アクセス時も維持されます
・ 年度を変更するにはTOPメニューに戻り、年度セレクタで選択してください

【メニュー画面（8つの機能タイル + サブメニュー）】

1.「仕訳入力」
  ・ 日付/借方科目/貸方科目/税込金額/取引先/元帳摘要/課税区分/税率を入力して仕訳を登録
  ・ 「AI自動判定」ボタン: 取引先と摘要と金額を入力して押すと、借方・貸方・課税区分・税率をAIが自動推定
  ・ 借方・貸方はオートコンプリート付き（登録済み勘定科目から候補表示）
  ・ 前年以前の日付を入力すると警告が出て、当年1月1日に自動修正（実日付は摘要に記録）

2.「証憑読み取り」
  ・ レシート・請求書・CSV明細をドラッグ&ドロップまたはファイル選択でアップロード
  ・ 対応形式: 画像（JPG/PNG等）、PDF、CSV
  ・ AIが自動解析して仕訳候補を作成、確認して「仕訳を登録」で一括保存
  ・ 「AI一括判定」ボタン: 解析結果の取引先・摘要の内容からAIが借方/貸方科目と消費税区分を再判定します。科目を手動修正した後でも再判定可能です
  ・ 科目を手動変更すると消費税区分も自動連動します（事業主勘定→不課税、費用科目→その科目のデフォルト税区分を適用）
  ・ 「Googleドライブから読み取り」ボタン: Drive内のinboxフォルダから一括取り込み、処理後はprocessedフォルダに自動移動
  ・ CSV明細は12社に対応（自動判定）:
    銀行7行: みずほ銀行、三菱UFJ銀行、三井住友銀行、ゆうちょ銀行、楽天銀行、住信SBIネット銀行、PayPay銀行
    カード5社: 楽天カード、三井住友カード、JCBカード、Amazonカード、アメリカン・エキスプレス
  ・ CSV文字コードはCP932/Shift_JIS/UTF-8を自動判定
  ・ CSVインポート時に口座初期設定バナーが表示されます。借方・貸方科目を設定し「保存」にチェックすると次回以降自動適用されます
  ・ 同じCSVファイルの2回目以降のインポートは重複警告が表示される（SHA-256ハッシュで検出）
  ・ 日付・金額・取引先が一致する既存の仕訳がある場合、重複候補として黄色く強調表示されます

3.「仕訳帳」
  ・ 登録済み仕訳の一覧表示（20件/ページのページネーション）
  ・ 期間切替: 「年初〜当日」「年間」「当月」、月の前後移動ボタン
  ・ 詳細検索: 日付範囲・勘定科目・金額範囲・取引先・摘要でフィルタリング
  ・ 編集: 仕訳行の「✎」ボタン → 編集モーダルで各項目を修正 →「保存」
  ・ 削除: 仕訳行の「×」ボタン → 確認後に削除
  ・ 一括削除: 各行のチェックボックスで選択し「一括削除」ボタンで複数仕訳をまとめて削除できます。ヘッダーのチェックボックスで全選択/解除が可能です

4.「勘定科目残高」
  ・ 3つのサブタブ: 「B/S 資産」「B/S 負債・純資産」「損益計算書」
  ・ 各科目の残高を期間別に表示
  ・ 科目行をクリック → その科目の明細（いつ・いくら動いたか）をドリルダウン表示
  ・ 明細内でも仕訳の編集・削除が可能

5.「取引先」 ・取引先の登録・編集・削除。仕訳入力時のオートコンプリートに反映される

6.「勘定科目」 ・勘定科目の追加・削除。仕訳で使用中の科目は削除不可

7.「消費税」
  ・ 3つのサブタブ: 「設定」「科目別消費税一覧」「消費税計算」
  ・ 設定: 課税事業者/免税事業者の選択、本則課税/簡易課税の選択、簡易課税の場合は事業区分（第1〜6種）を設定
  ・ 科目別消費税一覧: 勘定科目ごとに税率別の消費税額を集計表示。科目をクリックすると該当する仕訳の明細を表示でき、そこから編集も可能です
  ・ 消費税計算: 本則課税と簡易課税それぞれの納付税額を自動計算・比較できます。どちらが有利か確認できます
  ・ 税込経理方式で計算します

8.「期首残高設定」 ・会計年度の期首（1月1日時点）の各科目残高を設定。前年度の繰越額を入力する

【サブメニュー（メニュー画面下部の「...」ボタン）】
  ・ データのバックアップ: JSONダウンロード、Google Driveバックアップ、復元（現在のデータを完全上書き、注意！）
  ・ 固定資産台帳: 固定資産の登録・減価償却計算（定額法・定率法）・売却/除却処理・仕訳自動生成に対応
...
【アウトプット（右上ボタン）】
  ...
  ・ 貸借対照表(B/S): CSV / PDF出力
  ・ 損益計算書(P/L): CSV / PDF出力
  ・ 青色申告決算書: 確定申告用の4ページ決算書を一括出力（損益計算書・月別売上仕入・減価償却費等・貸借対照表）
  ・ 出力期間を選択してから各ボタンをクリック

【よくある質問への正しい回答】
Q: APIキーは必要？ → はい、必須です。Google AI StudioでGemini APIキーを無料で取得し、設定画面で登録してください
Q: 無料で使える？ → はい、Hinakira会計は完全無料です。Gemini APIも無料枠があります
Q: データはどこに保存される？ → 仕訳データはサーバーのデータベース、証憑画像はGoogle Driveに保存されます
Q: 確定申告に使える？ → はい、青色申告決算書（4ページ）の出力に対応しています
Q: 消費税の計算は？ → 「消費税」メニューで事業者区分と課税方式を設定後、「科目別消費税一覧」で科目ごとの消費税を確認、「消費税計算」で本則・簡易の納付税額を比較できます
Q: 仕訳を間違えた → 仕訳帳の「✎」ボタンから編集、または「×」ボタンで削除できます。複数まとめて削除する場合はチェックボックスで選択して「一括削除」ボタンを使ってください
Q: 年度を変えるには？ → TOPメニュー画面の左上にある年度セレクタで年度を選択してください。選んだ年度が全画面に反映されます
Q: 固定資産の減価償却は？ → サブメニュー（メニュー画面下部の「...」ボタン）から「固定資産台帳」を開き、資産を登録すると減価償却費を自動計算し、仕訳も自動生成できます
Q: 証憑の科目判定が間違っている → 手動で科目を修正するか、「AI一括判定」ボタンで取引先・摘要から科目と消費税を再判定できます。科目変更時は消費税区分も自動で連動します
