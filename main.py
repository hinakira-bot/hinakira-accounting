...
7.「期首残高設定」 ・会計年度の期首（1月1日時点）の各科目残高を設定。前年度の繰越額を入力する

8.「データのバックアップ」
  ・ JSONダウンロード: PCにバックアップファイルを保存
  ・ Google Driveバックアップ: Driveに自動保存
  ・ 復元: JSONファイルまたはDriveから復元（現在のデータを完全上書き、注意！）
...
Q: 消費税の計算は？ → 税込経理方式で、仕訳登録時に税区分（10%/8%/非課税/不課税）を設定すると自動計算されます
Q: 仕訳を間違えた → 仕訳帳の「✎」ボタンから編集、または「×」ボタンで削除できます
